...
function StartsRatingGroup({
  items,
  value,
  className,
  variant,
}: StartsRatingGroupProps & HTMLAttributes<HTMLDivElement>) {
  const [variantValue, setVariantValue] = useState<string>("");
  const selectedValue = parseInt(value);

  const textStyle = `text-${variant}`;
  const fillStyle = `${
    variant === "hooters" ? "hsl(var(--hooters))" : "hsl(var(--gus))"
  }`;
  return (
    <ul
      className={cn(
        className,
        "grid gap-1 sm:gap-2 text-sm font-medium text-gray-900 pb-3"
      )}
    >
