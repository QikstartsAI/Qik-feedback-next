...
import { getFirebase } from "@/lib/lib/firebase";
import { doc, getDoc } from "firebase/firestore";

export const findCustomerDataByEmail = async (
  email: string
...
export const findIsCustomerInBusiness = async (
  email: string,
  businessId: string
) => {
  const db = getFirebase().db;
...
export const findCustomerFeedbackDataInBusiness = async (
  email: string,
  businessId: string
) => {
  const db = getFirebase().db;
  ...
  return (await getDoc(customerDocRef)).data() as {
    customerNumberOfVisits: number;
  };
};

export const getCustomerDataInBusiness = async (
  email: string,
